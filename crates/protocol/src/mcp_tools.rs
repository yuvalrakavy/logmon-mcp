...
pub const TOOLS: &[Tool] = &[
    ...
    Tool {
        ...
        method: "logs.fields",
        description: "Map the log buffer BEFORE filtering or grouping: every field with coverage, distinct count, top values, type — and the `selector`, which is the DSL spelling that actually reaches that field. Reach for this first when you do not know what is in the buffer: a field name that does not exist returns an empty result rather than an error. ALWAYS filter with a row's `selector`, never with its `field` name — they differ. GELF strips the leading underscore, so `_file` becomes an additional field named `file` sitting beside the built-in `file`; they are different fields with different selectors (`file` vs `fi`) and both appear as separate rows. A `selector` of null means NO filter reaches that field at all — true of trace_id and span_id, for which you want get_recent_logs(trace_id=...). A field at 0% coverage exists but is never populated, the common case for the top-level GELF built-ins (fi/ln/fa). `kind` says which fields a numeric aggregation could sum. Walks the whole ring, not the newest N. Cursor qualifiers are rejected before resolution, so a refusal leaves nothing behind.",
        cli: Cli::PLAIN,
    },
    Tool {
        name: "profile_logs",
    ...
    Tool {
        name: "create_case",
        method: "cases.create",
        description: "Capture a window of logs and spans around an anchor as three files on disk: a markdown document you read to decide whether this is your bug, and two JSONL evidence files you consult once you have decided it is. Requires an absolute dir and a reason. The anchor is tagged — {seq}, {bookmark} or {trace_id} — and an unresolvable one is an error rather than a document with no headline. The document leads with what could NOT be captured: whether a session filter was narrowing what got stored over that window, whether the ring had evicted part of it, and which provenance keys are missing. Pass data to record provenance in the same call; a key with a leading @ is asserted about this capture alone and never enters the domain registry. before/after count stored RECORDS FROM EITHER STORE, not seq distances and not logs alone, so before=50 may return 40 logs and 10 spans — a span is exported when it ENDS, so the spans of a slow operation carry seqs above the last log about it and a log-only window would miss exactly the ones you captured the case for. Writes ONE <stem>.case.zip by default -- three loose files desync or lose a sibling in transit -- holding the document, both evidence files and the machine-readable registry; load_case reads it back. separate writes loose files instead, uncompressed stores rather than deflates (and errors with separate, where it would be inert), and omit_logdata / omit_spandata leave that evidence out. An omitted file has NO front-matter key, which is what tells a reader 'nobody shipped it' from 'we looked and found none' -- do not report 'no slow spans' from a case whose spans were omitted.",
        cli: Cli::PLAIN,
    },
    Tool {
        name: "get_trace_logs",
...
mod tests {
    ...
    fn every_pair_is_well_formed_and_unique() {
        // Pinned so a tool cannot be added here silently. The other half is
        // `crates/mcp/tests/mcp_stdio.rs`, which drives the real shim over

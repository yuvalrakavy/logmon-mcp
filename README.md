...
Three of these answer three different questions about a buffer, and reaching for
the wrong one is the usual way an investigation stalls:

| Question | Tool |
|---|---|
| *What dimensions exist in here?* | `list_log_fields` |
| *How do records distribute along **this** dimension?* | `profile_logs` |
| *Show me the records* | `get_recent_logs` |

The first is the map, and it comes first for a reason: naming an axis that
doesn't exist returns an empty result rather than an error, so guessing costs a
silent wrong answer.

| Tool | Description |
|---|---|
| `list_log_fields` | Map the buffer before querying it: every field with coverage, distinct count, top values, type — and the **filter selector that actually reaches it**, which is not the same as its name. GELF strips the `_` prefix, so `_file` becomes an additional field named `file` sitting beside the built-in `file`; they are different fields with different selectors (`file` vs `fi`) and both appear as separate rows. **A field at 0% coverage exists but is never populated.** A `null` selector means no log filter reaches the field at all (`trace_id`/`span_id` — use `get_recent_logs(trace_id=…)`). Walks the whole ring, not the newest N. Cursor qualifiers are refused, before resolution, so a refusal leaves nothing behind. |
| `profile_logs` | How records **distribute** along one axis, once `list_log_fields` has told you which axes exist: counts, a per-level breakdown, seq/time bounds, and a verbatim exemplar at each end of every group. `group_by` takes a built-in (`level`, `message`, `host`, `facility`, `file`, `line`, `trace_id`, `span_id`) or `field` plus `group_keys` naming emitter fields; two keys form a tuple. **`__absent__` is a normal row and usually the largest** — most axes are absent from over 90% of records, and the row is what makes the counts account for every matched record. It sorts last and does not consume `top_n`. **`__overflow__` is a different fact** (the cardinality cap folded keys) and the two are never merged. Identify either by the row's **`reserved`** field, never by its key: GELF keeps whatever follows the `_`, so a field *valued* `__absent__` is an ordinary row whose key is identical to the bucket's and which sorts ahead of it. Rows sum to `matched` only when `groups_total` is at most `top_n`. A `suppressed` entry means the axis appears on no matched record — usually a spelling problem, and the remedy names the call that works. Counts only; no sums or averages yet. |

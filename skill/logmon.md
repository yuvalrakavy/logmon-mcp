...
In CLI mode there is no `use` verb — pass the global `--domain NAME` flag to scope one invocation (e.g. `logmon-mcp --domain t3 logs recent`); it resets to `default` when omitted. Durable domains can be declared in `config.json`.

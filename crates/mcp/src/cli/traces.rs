...
use logmon_broker_protocol::{
    LogEntry, SpanEntry, TraceSummary, TracesGet, TracesLogs, TracesRecent, TracesSlow,
    TracesSummary,
};
    ...
    match cmd.verb {
        TrcVerb::Recent { count, filter } => {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    format::error(&format!("traces.recent failed: {e}"), json);
                    return 1;
                }
            };
            if json {
        ...
        TrcVerb::Get {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    format::error(&format!("traces.get failed: {e}"), json);
                    return 1;
                }
            };
            if json {
            ...
            println!(
                "trace: {} ({} spans, {} logs)",
                result.trace_id, result.span_count, result.log_count
            );
        ...
        TrcVerb::Summary { trace_id } => {
            let result = match broker.traces_summary(TracesSummary { trace_id }).await {
                Ok(r) => r,
                Err(e) => {
                    format::error(&format!("traces.summary failed: {e}"), json);
                    return 1;
                }
            };
            if json {
            ...
            println!(
                "total: {:.1}ms ({} spans)",
                result.total_duration_ms, result.span_count
            );
        ...
        TrcVerb::Slow {
                ...
                .await
            {
                Ok(r) => r,
                Err(e) => {
                    format::error(&format!("traces.slow failed: {e}"), json);
                    return 1;
                }
            };
            if json {
        ...
        TrcVerb::Logs { trace_id, filter } => {
            let result = match broker.traces_logs(TracesLogs { trace_id, filter }).await {
                Ok(r) => r,
                Err(e) => {
                    format::error(&format!("traces.logs failed: {e}"), json);
                    return 1;
                }
            };
            if json {

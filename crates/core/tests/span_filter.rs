...
fn test_parse_span_name_selector() {
    ...
    } else { panic!("expected Qualifiers"); }
}
...
fn make_span(name: &str, service: &str, duration_ms: f64, status: SpanStatus) -> SpanEntry {
    let now = Utc::now();
    SpanEntry {
        seq: 0, trace_id: 0xabc_u128, span_id: 0xdef_u64,
        parent_span_id: None,
        start_time: now, end_time: now,
        duration_ms, name: name.to_string(),
        kind: SpanKind::Server,
        service_name: service.to_string(),
        status, attributes: HashMap::new(), events: vec![],
    }
}
...
fn test_match_service_name() {
    let f = parse_filter("sv=store_server").unwrap();
    assert!(matches_span(&f, &make_span("op", "store_server", 10.0, SpanStatus::Ok)));
}
...
fn test_match_duration_gte() {
    ...
    assert!(matches_span(&f, &make_span("op", "svc", 100.0, SpanStatus::Ok)));
}

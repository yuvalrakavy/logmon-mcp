...
async fn bookmarks_add_then_list_round_trip() {
    ...
    assert!(
        add_stdout.contains("my-anchor") || add_stdout.contains("seq=42"),
        "got: {add_stdout}"
    );
    ...
    assert!(
        entries.iter().any(|b| b["seq"] == 42),
        "expected anchor with seq=42; got: {v}"
    );
}
...
async fn bookmarks_remove_succeeds() {
    ...
    let rm = tokio::task::spawn_blocking(move || {
        rm_cmd
            .args(["bookmarks", "remove", "--name", "cli/to-rm"])
            .output()
            .unwrap()
    })
    .await
...
async fn bookmarks_add_replace_overwrites() {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        first_cmd
            .args(["bookmarks", "add", "--name", "x", "--start-seq", "1"])
            .output()
            .unwrap()
    })
    .await
    ...
    let dup = tokio::task::spawn_blocking(move || {
        dup_cmd
            .args(["bookmarks", "add", "--name", "x", "--start-seq", "2"])
            .output()
            .unwrap()
    })
    .await
